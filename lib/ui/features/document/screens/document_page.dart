...
class _PayRollState extends State<PayRoll> {
  ...
  _getlistPayRoll() async {
    ...
    try {
      ...
      List<Document> newDocuments = await con.documentList(
          userSession?.userId, _currentPage, monthFinal, yearFinal,userSession?.companyId.toString());
  ...
  Future<void> _transformAndAddData(List<Document> newDocument) async {
    newDocument.forEach((document) {
      String dateString = document.createDate;
      ...
      transformedList.add({
        ...
        'widthsignature': document.userWidth ?? true,
        'heightsignature': document.userHeight ?? true
      });
    });
  }
                                                            ...
                                                            TextButton(
                                                                onPressed:
                                                                  ...
                                                                  if (activeButton) {
                                                                    ...
                                                                    Get.back();
                                                                    var signatureData = await signatureController.toPngBytes();
                                                                    String base64String = base64Encode(signatureData!);
                                                                     showProgressDialog(context,base64String,element);
  ...
  BuildContext? _dialogContext; // Variable global para almacenar el contexto del diálogo

  void showProgressDialog(BuildContext context, String signature, dynamic element) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext dialogContext) {
        _dialogContext = dialogContext; // Guardamos el contexto del diálogo
        return AlertDialog(
          backgroundColor: Colors.white,
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              CircularProgressIndicator(color: mainColorBlue),
              SizedBox(height: 10),
    ...
    _signDocumentAndUpdateList(context, signature, element);
  }

  Future<void> _signDocumentAndUpdateList(BuildContext parentContext, String signature, dynamic element) async {
    try {
      await DocumentProvider().signatureDocument(
        element['id_document'],
        signature,
      );
      ...
      if (_dialogContext != null && _dialogContext!.mounted) {
        ...
        _dialogContext = null; // Limpiar el contexto después de cerrarlo
      }
      ...
      if (parentContext.mounted) {
        ...
        ScaffoldMessenger.of(parentContext).showSnackBar(
          const SnackBar(content: Text('Documento firmado')),
        );
      }
    } catch (e) {
      print("Error: $e");
